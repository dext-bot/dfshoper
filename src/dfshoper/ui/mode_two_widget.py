...
class ModeTwoWidget(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self._threshold_input.setPlaceholderText("请输入阈值价格")
        validator = QDoubleValidator(0.0, 1_000_000_000.0, 3, self._threshold_input)
        validator.setNotation(QDoubleValidator.StandardNotation)
        self._threshold_input.setValidator(validator)
    ...
    def _emit_config(self) -> None:
        ...
        if threshold_text and not self._threshold_input.hasAcceptableInput():
            self._threshold_input.clear()
            self._threshold_input.setPlaceholderText("请输入有效数字")
            self._threshold_input.setFocus()
            return
        try:
